...
(function($){
	 ...
	 * Condition Editor
	 */
	$.widgetsFactory( "bpConditionEditor", {
		...
		_create: function() {
			...
			this._$editor.on('filters-changed', function(){
				self._checkIsEmpty();
			});
		...
		addFilter: function( data ) {
			...
			self._$editor.trigger('filters-changed');
			return $filter;
		},
	 ...
	 * Condition Filter
	 */
	$.widgetsFactory( "bpConditionFilter", {
		...
		_create: function() {
			...
			this._$filter.on('conditions-changed', function(){
				self._checkIsEmpty();
			});
		},
		...
		addCondition: function( data, $scope ) {
			...
			if ( data.type === 'scope' ) {
				...
				this._$filter.trigger('conditions-changed');
			}
		},
	...
	$.widgetsFactory( "bpCondition", {
		...
		createValueAsInteger: function( paramOptions, isInit ) {
			...
			$operator.on('change', function(){
				...
				var $control;
				if ( 'between' === currentOperator ) {
					$control = $("<span><input type='text' class='bp-integer bp-integer-start form-control' /> and <input type='text' class='bp-integer bp-integer-end' /></span>");
				} else {
					$control = $("<input type='text' class='bp-integer bp-integer-solo form-control' /></span>");
				}
		...
		createValueAsText: function( paramOptions, isInit ) {
			...
			var $control = $("<input type='text' class='bp-text form-control' /></span>");
			this.insertValueControl( $control );
		...
		getParamOptions: function( paramName ) {
			...
			console.log(window.bp.filtersParams);
		...
		createDataPircker: function() {
			...
			var $input = $('<input size="16" type="text" class="form-control" readonly="readonly" />');
