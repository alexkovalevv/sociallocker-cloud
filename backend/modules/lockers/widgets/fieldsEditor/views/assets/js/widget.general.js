        ...
        $.widgetsFactory("fieldsEditor", {
            ...
            _typesList: [
                {name: 'text', title: 'Текст', editorName: 'text'},
                {
                    ...
                    title: 'Email',
                    editorName: 'text',
                    placeholder: window.fieldsEditor.res['email-field-placeholder']
                },
                {
                    name: 'phone',
                    title: 'Телефон',
                    editorName: 'text',
                    placeholder: window.fieldsEditor.res['phone-field-placeholder']
                },
                {name: 'url', title: 'Вебсайт/URL', editorName: 'text', placeholder: 'http://'},
                {
                    ...
                    title: 'День рожденья',
                    editorName: 'text',
                    placeholder: window.fieldsEditor.res['birthday-field-placeholder']
                },
                ...
                {name: 'unsupported', title: window.fieldsEditor.res['unsupported'], hide: true},
            ...
            initMappingColumn: function () {
                var self = this,
                    listIdFieldName = window.fieldsEditor.listIdFieldName
            ...
            refreshMappingSelectors: function (listId) {
                ...
                var setErrorStatus = function () {
                    ...
                    self.lockSelects($selectors, window.fieldsEditor.res['error-state']);
                };
                var def = self.asyncGetMappingFields(listId);
                def.done(function (data) {
                    if (!data || data.error) {
                        setErrorStatus();
                        return self.showError(( data && data.error ) || window.fieldsEditor.res['unexpected-error']);
                    }
                ...
                $selectors.each(function () {
                    ...
                    if (self.isEmailField($fieldRow)) {
                        options.push({
                            value: 'email',
                            title: 'Email',
                            hint: window.fieldsEditor.res['email-field-hint'],
                            selected: true
                        });
                    } else {
                        var subOptions = [];
                        ...
                        options.push({
                            title: window.fieldsEditor.res['service-title'],
                            items: subOptions
                        });
                        options.push({
                            title: 'Умные поля',
                            items: [{
                                value: 'fullname',
                                title: 'Полное имя',
                                hint: window.fieldsEditor.res['fullname-field-hint'],
                                data: {
                                    id: 'fullname',
                                    req: true,
                                    placeholder: window.fieldsEditor.res['fullname-field-placeholder']
                                }
                            }]
                        });
                        options.push({
            ...
            asyncGetMappingFields: function (listId) {
                ...
                if( !window.fieldsEditor.ajaxUrl )
                    return;
                ...
                var req = $.ajax({
                    type: 'GET',
                    url: window.fieldsEditor.ajaxUrl,
                    dataType: 'json',
                    data: {
                       list_id: listId
                    },
                    error: function () {
                        self.showError(window.fieldsEditor.res['unexpected-error']);
                        console.log("Unexpected error occured during the ajax request.");
                        console.log(req.responseText);
                    }
                });
                this.lockSelects($selectors, window.fieldsEditor.res['loading-state'], req);
                return req;
            },
            // --------------------------------------------------------
            // Column :: Field Type
            ...
            refreshTypeSelector: function ($fieldRow) {
                ...
                if (options.length === 0) {
                    ...
                    var option = {
                        ...
                        hint: window.fieldsEditor.res['unsupported-type']
                    };
                    options.push(option);
            ...
            onUpdatingTypeSelector: function ($fieldRow) {
                ...
                if ('checkbox' === fieldType) {
                    if (!fieldOptions.description) typeFieldOptions.description = window.fieldsEditor.res['checkbox-default-description'];
                ...
                if ('label' === fieldType && !fieldOptions.text)
                    typeFieldOptions.text = window.fieldsEditor.res['label-default-text'];
