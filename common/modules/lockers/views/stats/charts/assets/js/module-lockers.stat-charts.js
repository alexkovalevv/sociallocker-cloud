	...
	window.bizpanda.statistics = {
		...
		init: function() {
			...
			$('.wt-dropdown-select').change(function(e) {
				if( $(e.target).attr('name') === 'locker_id' || $(e.target).attr('name') === 'locker_type' ) {
					$('#opanda-item-selector').submit();
				}
			});
		...
		initButtonSelector: function() {
			...
			$(".onp-sl-selector-item").addClass('opanda-inactive')
				.removeClass('opanda-active');

			/*$('th[class^="opanda-column-unlock-via-"], td[class^="opanda-col-unlock-via-"]')
			 .hide();*/
			...
			for( var index in activeSelectors ) {
				...
				/*$(".opanda-col-" + activeSelectors[index])
				 ...
				 $(".opanda-column-" + activeSelectors[index])
				 .show();*/
			}
		...
		savePreSelectedButtons: function() {
			...
			localStorage.setItem('opanda-chart-selectors', JSON.stringify(buttons));
		},
