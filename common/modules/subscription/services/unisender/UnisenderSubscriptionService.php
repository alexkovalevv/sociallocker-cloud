<?php
namespace common\modules\subscription\services\unisender;
...
class UnisenderSubscriptionService extends Subscription {
	...
	public function subscribe( $identityData, $listId, $doubleOptin, $contextData, $verified ) {
		...
		if ( empty( $vars['Name'] ) && !empty( $identityData['name'] ) ) $vars['Name'] = $identityData['name'] .
			(!empty($identityData['family']) ? ' ' . $identityData['family'] : '');

		$response = $this->callApi('subscribe', array(
			'list_ids'     => $listId,
			'fields'       => $vars,
			'request_ip'   => $this->getClientIp(),
			'double_optin' => $verified ? 1 : ( $doubleOptin ? 0 : 1 ),
			'overwrite'    => 0
		));
	...
	//get data by method
	public function callApi($method = null, array $params = array()) {

		$apikey = Yii::$app->lockersSettings->get('unisender_api_key', false);
