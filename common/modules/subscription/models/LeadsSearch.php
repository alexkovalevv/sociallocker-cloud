<?php
	...
	class LeadsSearch extends Leads {
		...
		{
			return [
				[['id', 'user_id', 'lead_email_confirmed', 'lead_subscription_confirmed', 'locker_id'], 'integer'],
				[
					[
						'user_id',
						'lead_display_name',
		...
		public function search($params)
		{
			...
			$query = Leads::find()->where(['locker_id' => $range]);
			...
			$query->andFilterWhere(['like', 'lead_display_name', $this->lead_display_name])->andFilterWhere([
				'like',
				'lead_name',
				$this->lead_name
			])->andFilterWhere(['like', 'lead_family', $this->lead_family])->andFilterWhere([
				'like',
				'lead_email',
				$this->lead_email
			])->andFilterWhere(['like', 'lead_item_title', $this->lead_item_title])->andFilterWhere([
				'like',
				'lead_referer',
				$this->lead_referer
			])->andFilterWhere([
