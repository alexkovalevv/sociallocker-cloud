<?php
	...
	class LeadsHelper {
		 ...
		 */
		public static function add($identity = [], $context = [], $emailConfirmed = false, $subscriptionConfirmed = false, $temp = null)
		{
			$lockerId = isset($context['lockerId'])
				? intval($context['lockerId'])
				: 0;

			if( !empty($lockerId) && isset($identity['source']) && !Yii::$app->locker->getOption($lockerId, $identity['source'] . '_lead_available', true) ) {
				return false;
			}
		...
		public static function saveAvatar($lead_id)
		{
			...
			if( !$lead_fields_model->updateLeadField($lead_id, '_image_x40', $lead_id . '_x40.jpeg') || !$lead_fields_model->updateLeadField($lead_id, '_image_x500', $lead_id . '_x500.jpeg') ) {
				return false;
			}
