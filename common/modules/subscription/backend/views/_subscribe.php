<?php
	/**
	 * Шаблон подписки на сервисы почтовых рассылок. Часть шаблона общих настроек.
	...
	$fields->model = $models->getModel('subscribe');

	$services = Yii::$app->subscription->getSerivcesList();
	...
	// Default
	// ========================================================================================
	$database_fieldset = $fields->textInput('service_sender_email');
	$database_fieldset .= $fields->textInput('service_sender_name');
	$database_fieldset .= $fields->textInput('service_confirm_email_subject');
	$database_fieldset .= $fields->editor('service_confirm_email_body');

	$subscribe .= Html::tag('div', $database_fieldset, [
		'class' => 'fieldset-hidden database-fieldset'
	]);
