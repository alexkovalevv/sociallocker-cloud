...
class OPanda_OAuthApplication implements OPanda_AWeberOAuthAdapter {
    ...
    public function getRequestToken($callbackUrl=false) {
        $data = ($callbackUrl)? ['oauth_callback' => $callbackUrl] : [];
        $resp = $this->makeRequest('POST', $this->app->getRequestTokenUrl(), $data);
        $data = $this->parseResponse($resp);
        $this->requiredFromResponse($data, ['oauth_token', 'oauth_token_secret']);
    ...
    public function getAccessToken() {
        ...
        $resp = $this->makeRequest('POST', $this->app->getAccessTokenUrl(),
            ['oauth_verifier' => $this->user->verifier]
        );
    ...
    public function makeRequest($method, $url, $data=[]) {
        ...
        switch (strtoupper($method)) {
            ...
            case 'PATCH':
                $oauth = $this->prepareRequest($method, $url, []);
        ...
        if (!$resp) {
            ...
            $error = ['message' => $msg, 'type' => 'APIUnreachableError',
    ...
    protected function patch($url, $oauth, $data) {
        ...
        $resp = $this->_sendRequest($handle, ['Expect:', 'Content-Type: application/json']);
        return $resp;
    }
     ...
     * @return void
     */
    private function _sendRequest($handle, $headers = ['Expect:']) {
