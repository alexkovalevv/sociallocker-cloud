...
class OPanda_AweberSubscriptionService extends OPanda_Subscription {
    ...
    public function getCredentialUsingAuthorizeKey($authorize_key) {
        ...
        if ( empty( $consumer_key) || empty( $consumer_secret) || empty( $access_key ) || empty( $access_secret ) ) {
            throw new OPanda_SubscriptionException( __('Unable to connect your Aweber Account. The Authorization Code is incorrect.', 'bizpanda') );
        }
    ...
    public function subscribe( $identityData, $listId, $doubleOptin, $contextData, $verified ) {
        ...
        try {
            ...
            return ['status' => $doubleOptin ? 'pending' : 'subscribed'];
